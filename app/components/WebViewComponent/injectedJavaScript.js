 ...
 *
 *  webViewBridge.send('functionToInvoke', {mydata: 'test'}, function(){writeMsg('success')},function(){writeMsg('error')});
 *
...
const injectedJavaScript = function(){
   ...
   var init = function() {
           ...
           send: function(targetFunc, data, success, error) {
               ...
               promiseChain = promiseChain.then(function () {
                   return new Promise(function (resolve, reject) {
                       writeMsg("sending message " + msgObj.targetFunc);
                   ...
                   writeMsg('rnBridge send failed ' + e.message);
               });
           },
       ...
       window.document.addEventListener('message', function(e) {
           writeMsg("message received from react native");
           ...
           try {
               ...
               writeMsg("failed to parse message from react-native " + err);
               return;
           }
   ...
   try {
    init();
    window.webViewBridge.send('abc', {}, function(){
        writeMsg('success');
    }, function(){
        writeMsg('error');
    });
    writeMsg('init done 2!');
   } catch (e) {
        writeMsg(e.message);
   }
...
//
const injectedJSContent = '(' + injectedJavaScript.toString() + ')()';
export default injectedJSContent;
